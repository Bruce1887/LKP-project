...
#define T_SMALL_NAME "tsmall.txt"

int truncate_small_to_small_file(void)
{
	int ret;
	FILE *file = fopen(OUICHEFS_FILE_NAME(T_SMALL_NAME), "w");
	if (!file)
		return ERR_CREATE;
	...
	file = fopen(OUICHEFS_FILE_NAME(T_SMALL_NAME), "w");
	if (!file)
		return ERR_CREATE;
	...
	file = fopen(OUICHEFS_FILE_NAME(T_SMALL_NAME), "r");
	if (!file)
		return ERR_OPEN;
